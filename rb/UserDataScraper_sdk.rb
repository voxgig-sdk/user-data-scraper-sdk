...
require_relative 'UserDataScraper_types'
...
class UserDataScraperSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = UserDataScraperHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = UserDataScraperHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, UserDataScraperFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    UserDataScraperUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = UserDataScraperHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = UserDataScraperHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = UserDataScraperHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    rescue UserDataScraperError => err
    ...
    if fetched.is_a?(Hash)
      status = UserDataScraperHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = UserDataScraperError.new(
        "graphql_error", "UserDataScraperSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = UserDataScraperSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
