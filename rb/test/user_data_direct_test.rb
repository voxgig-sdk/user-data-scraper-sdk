...
class UserDataDirectTest < Minitest::Test
  def test_direct_list_user_data
    setup = user_data_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-user_data", setup[:live] ? "live" : "unit")
    if _should_skip
...
def user_data_direct_setup(mockres)
  ...
  if live
    ...
    client = UserDataScraperSDK.new(merged_opts)
    return {
      client: client,
      calls: calls,
      live: true,
