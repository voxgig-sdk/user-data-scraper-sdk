...
class UserDataEntity
  ...
  def make
    opts = @_entopts.dup
    UserDataEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = UserDataScraperHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [UserData, Hash] the current UserData data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = UserDataScraperHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of UserData fields)
  def match_get
  ...
  #
  # @param reqmatch [UserDataListMatch, Hash, nil] match filter (any subset of
  #   UserData fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<UserData>, Array] the matching UserData items; raises UserDataScraperError on failure
