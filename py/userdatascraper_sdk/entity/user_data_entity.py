...
from userdatascraper_sdk.userdatascraper_types import (
    UserData,
    UserDataListMatch,
)
...
class UserDataEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return UserDataEntity(self._client, opts)
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[UserData]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.UserData().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
