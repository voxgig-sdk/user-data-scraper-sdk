# UserDataScraper SDK test runner
...
class UserDataScraperTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            UserDataScraperTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = UserDataScraperTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = UserDataScraperTestRunner.getenv("USER_DATA_SCRAPER_TEST_LIVE")
        override = UserDataScraperTestRunner.getenv("USER_DATA_SCRAPER_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = UserDataScraperTestRunner.getenv(key)
        ...
        explain = UserDataScraperTestRunner.getenv("USER_DATA_SCRAPER_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["USER_DATA_SCRAPER_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if UserDataScraperTestRunner._test_control is not None:
            return UserDataScraperTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                UserDataScraperTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            UserDataScraperTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = UserDataScraperTestRunner.load_test_control()
...
def load_env_local():
    UserDataScraperTestRunner.load_env_local()


def env_override(m):
    return UserDataScraperTestRunner.env_override(m)


def entity_data(v):
    return UserDataScraperTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return UserDataScraperTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return UserDataScraperTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return UserDataScraperTestRunner.load_test_control()


def live_client_options():
    return UserDataScraperTestRunner.live_client_options()


def live_delay_ms():
    return UserDataScraperTestRunner.live_delay_ms()
