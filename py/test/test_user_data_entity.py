...
class TestUserDataEntity:

    def test_should_create_instance(self):
        testsdk = UserDataScraperSDK.test(None, None)
        ent = testsdk.UserData(None)
    ...
    def test_should_stream(self):
        ...
        base = UserDataScraperSDK.test(seed, None)
        seen = list(base.UserData(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            sdk = UserDataScraperSDK.test(
                seed, {"feature": {"streaming": {"active": True}}})
            got = []
            for item in sdk.UserData(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        setup = _user_data_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["list"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "user_data." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set USER_DATA_SCRAPER_TEST_USER_DATA_ENTID JSON to run live")
        client = setup["client"]

        # Bootstrap entity data from existing test data.
        user_data_ref01_data_raw = vs.items(helpers.to_map(
            vs.getpath(setup["data"], "existing.user_data")))
        user_data_ref01_data = None
        if len(user_data_ref01_data_raw) > 0:
...
def _user_data_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/user_data/UserDataTestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("USER_DATA_SCRAPER_TEST_USER_DATA_ENTID"))
    if idmap_resolved is None:
        idmap_resolved = helpers.to_map(idmap)

    if env.get("USER_DATA_SCRAPER_TEST_LIVE") == "TRUE":
        merged_opts = vs.merge([
            {
            },
            extra or {},
        ])
        client = UserDataScraperSDK(helpers.to_map(merged_opts))

    _live = env.get("USER_DATA_SCRAPER_TEST_LIVE") == "TRUE"
    return {
        "client": client,
        "data": entity_data,
        "idmap": idmap_resolved,
        "env": env,
        "explain": env.get("USER_DATA_SCRAPER_TEST_EXPLAIN") == "TRUE",
        "live": _live,
