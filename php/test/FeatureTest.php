...
// UserDataScraper SDK feature test
//
...
class FtCtrl extends UserDataScraperControl
{
...
class FtClient
{
    ...
    private UserDataScraperUtility $utility;

    public function __construct(UserDataScraperUtility $utility, array $options)
    {
    ...
    public function get_utility(): UserDataScraperUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (UserDataScraperContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public UserDataScraperContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (UserDataScraperContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = UserDataScraperConfig::make_feature($name);
    ...
    private static function buildUrl(UserDataScraperSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof UserDataScraperError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new UserDataScraperResponse($fetched);
        }
    ...
    private function fail(UserDataScraperContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (UserDataScraperContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
