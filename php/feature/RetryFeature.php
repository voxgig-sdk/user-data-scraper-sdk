...
// UserDataScraper SDK retry feature
...
class UserDataScraperRetryFeature extends UserDataScraperBaseFeature
{
    ...
    public function init(UserDataScraperContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (UserDataScraperContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(UserDataScraperContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(UserDataScraperContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
