...
// UserDataScraper SDK netsim feature
...
class UserDataScraperNetsimFeature extends UserDataScraperBaseFeature
{
    ...
    private function _simulate(UserDataScraperContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
        $this->calls++;
        $call = $this->calls;
    ...
    private function _track(UserDataScraperContext $ctx, array $applied): void
    {
        $client = $this->client;
