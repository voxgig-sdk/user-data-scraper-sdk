...
boru-driven command-line client **and** interactive REPL for the UserDataScraper
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/user-data-scraper-cli)
make build

# 2. See usage (words, entities, env vars)
./user-data-scraper-cli --help
...
USER_DATA_SCRAPER_BASE=https://api.example.com ./user-data-scraper-cli list user_data

# 6. No arguments -> interactive REPL
./user-data-scraper-cli
user-data-scraper> list user_data
user-data-scraper> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/user-data-scraper-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/user-data-scraper-cli list user_data
   ```
...
Run with no arguments to open a REPL (prompt `user-data-scraper>`). Each line is
evaluated as its own boru expression:

```text
$ ./user-data-scraper-cli
user-data-scraper> list user_data
user-data-scraper> /help
user-data-scraper> /quit
```
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `user_data`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/user-data-scraper-cli`. |
...
(`./user-data-scraper-cli <expr>`) and interactively (the REPL), and expressions compose the
