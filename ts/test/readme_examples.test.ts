...
//     (\`new UserDataScraperSDK(...)\` / \`UserDataScraperSDK.test(...)\` become
//     \`UserDataScraperSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'UserDataScraperSDK'
...
const TEST_SEED = {"entity":{"user_data":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new UserDataScraperSDK(...)\` and any
// \`UserDataScraperSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+UserDataScraperSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/UserDataScraperSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
