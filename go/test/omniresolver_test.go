...
import (
	...
	omni "github.com/voxgig-sdk/user-data-scraper-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.UserDataScraperError); is {
				out := map[string]any{
					"name":    "UserDataScraperError",
...
func omniCtx(arg any, client *sdk.UserDataScraperSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.UserDataScraperSDK); is {
			client = live
