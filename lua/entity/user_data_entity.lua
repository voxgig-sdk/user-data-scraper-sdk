...
local UserDataEntity = {}
UserDataEntity.__index = UserDataEntity


function UserDataEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, UserDataEntity)
  self._name = "user_data"
  self._client = client
...
function UserDataEntity:get_name()
  return self._name
end


function UserDataEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return UserDataEntity.new(self._client, opts)
end
...
function UserDataEntity:mark_deleted()
  self._deleted = true
end


function UserDataEntity:deleted()
  return true == self._deleted
end


function UserDataEntity:data_set(args)
  if args ~= nil then
...
function UserDataEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function UserDataEntity:match_set(args)
  if args ~= nil then
...
function UserDataEntity:match_get()
...
function UserDataEntity:stream(action, args, callopts)
...
function UserDataEntity:_run_op(ctx, post_done)
  local utility = self._utility
