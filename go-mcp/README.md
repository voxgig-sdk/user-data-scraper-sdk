...
[MCP](https://modelcontextprotocol.io) server exposing the UserDataScraper SDK as
two agent tools — `user-data-scraper_list` and `user-data-scraper_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/user-data-scraper-mcp)
make build

# 2. Provide credentials via the environment
export USER_DATA_SCRAPER_APIKEY=sk_live_xxx
...
```jsonc
// user-data-scraper_list: first page of records
{ "entity": "user_data" }
{ "entity": "user_data", "query": { } }

// user-data-scraper_load: one record by id
{ "entity": "user_data", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/user-data-scraper-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user user-data-scraper \
     -- "$PWD"/dist/*/user-data-scraper-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `user-data-scraper_list` and `user-data-scraper_load` tools now appear
   in new sessions. Ask the agent to *"list user_data using user-data-scraper"*
   and it calls `user-data-scraper_list` with `{"entity":"user_data"}`.
...
```sh
export USER_DATA_SCRAPER_APIKEY=sk_live_xxx            # API key
export USER_DATA_SCRAPER_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `user-data-scraper_list` tool
...
### Call the `user-data-scraper_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `user-data-scraper_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `user-data-scraper_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `USER_DATA_SCRAPER_APIKEY` | API key sent with every request. |
| `USER_DATA_SCRAPER_BASE` | Optional override of the API base URL. |
