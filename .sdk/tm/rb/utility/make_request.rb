# UserDataScraper SDK utility: make_request
...
module UserDataScraperUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = UserDataScraperResponse.new({})
    ...
    if fetch_err
      ...
      response = UserDataScraperResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = UserDataScraperResponse.new(fetched)
    else
